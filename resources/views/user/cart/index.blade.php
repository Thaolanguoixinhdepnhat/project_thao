                            ...
                            <div class="item">
                                <div class="product-image-cart">
                                    @if ($item['product']->productImages->isNotEmpty())
                                        <img src="{{ asset('storage/' . $item['product']->productImages->first()->product_image) }}" alt="Image" class="active">
                                    @endif
                                </div>
                                <div class="product-txt">
                                    <h3>{{ $item['product']->name }}</h3>
                                    <span class="cart__options">{{ $item['product_class'] ? $item['product_class']->name : 'Mặc định' }}</span>
                                    <div class="cart-price">
                                        <span class="current-price">{{ number_format($item['product']->price, 0, ',', '.') }} VND</span>
                                    </div>
                                    <div class="cart__sku">Mã sản phẩm: {{ $item['product']->sku }}</div>
                                    <div class="cart__stock">Còn hàng</div>
                                </div>
                                <div class="cart-details">
                                    <div class="number-input">
                                        {{-- <form action="{{ route('cart.update') }}" method="POST">
                                            ...
                                            <input type="hidden" name="product_class_id" value="{{ $item['product_class_id'] }}">
                                            <button type="submit" name="quantity" value="{{ $item['quantity'] - 1 }}" class="minus">-</button>
                                            <span>{{ $item['quantity'] }}</span>
                                            <button type="submit" name="quantity" value="{{ $item['quantity'] + 1 }}" class="plus">+</button>
                                        </form> --}}
